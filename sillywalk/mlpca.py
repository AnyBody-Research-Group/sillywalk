...
class PCAPredictor:
    ...
    def __baseinit__(
        self,
        all_means: list[float] | np.ndarray,
        all_stds: list[float] | np.ndarray,
        all_columns: list[str] | np.ndarray,
        pca_columns: list[str] | np.ndarray,
        pca_components: list[float] | np.ndarray,
        pca_eigenvalues: list[float] | np.ndarray,
    ):
    ...
    def __init__(
        ...
        pca_columns = all_columns[np.logical_and(variances >= variance_threshold, relative_ratios >= relative_variance_ratio)]
    ...
    def _drop_parallel_constraints(self, B: np.ndarray, d: dict[str, float]) -> tuple:
        drop: list[str] = []
    ...
    def predict(
        ...
        low_variance_constraints = [col for col in constraints if col in self._low_variance_columns]
        if low_variance_constraints:
            raise ValueError(f"Constraint cannot be applied to excluded low-variance columns: {low_variance_constraints}")

        # constraint_indices = [self.pca_columns.get_loc(var) for var in constraints.keys()]
        constraint_indices = np.array([self.pca_columns[var] for var in constraints if var in self.pca_columns])

        standardized_constraints = {}
        for var in constraints:
            if var not in self.pca_columns:
                raise ValueError(f"Constraint variable '{var}' is not part of the PCA columns.")
            idx = self.pca_columns[var]
            standardized_constraints[var] = (constraints[var] - self._mean_vec[idx]) / self._std_vec[idx]
